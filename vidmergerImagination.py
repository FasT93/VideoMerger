...
class VideoMerger():
    def __init__(self, output_format='avi', size=None, shuffle=True,
                 ...
                 ready_duration=5, thanks_duration=5, transitionDuration = 0, fixed_classes=None,
    ...
    def merge(self, input_folder, output_folder='newvideo', depth=None, show=False):
        ...
        transImg = np.zeros((max_height, max_width, 3), np.uint8)
        transImg[:,:,0] = 50
        transImg[:, :, 2] = 50
            ...
            if (self.last_pause is True) or (self.last_pause is False and i + 1 < len(self.vids)):
                for t in range(current_pause_frames):
                    if t / frame_rate <= self.marker_duration:
                        ...
                        out.write(black)
            #adding transition
            current_trans_duration = self.transitionDuration
            current_trans_frames = int(frame_rate * current_trans_duration)

            if (self.last_pause is True) or (self.last_pause is False and i + 1 < len(self.vids)):
                for t in range(current_trans_frames):
                    out.write(self.__drawMarker(transImg, self.marker_size, self.marker_offset, color=(0, 0, 0)))

            # Saving duration
            # onsets.append(video_duration / frame_rate + current_pause_duration)
            onsets.append((video_duration + current_pause_frames + current_trans_frames) / frame_rate)
